...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .filters select, .filters input { padding: 0.4rem 0.7rem; border: 1px solid #d1d5db; border-radius: 6px; font-size: 0.85rem; background: #fff; }
    ...
    .error { text-align: center; padding: 2rem; color: #dc2626; background: #fef2f2; border-radius: 10px; margin-bottom: 1rem; }
  ...
  <div class="container">
    ...
    <div id="content" style="display:none">
      ...
      <div class="filters">
        ...
        <label for="parcelaFilter">Parcela:</label>
  ...
  <script>
    ...
    async function loadData() {
      try {
        ...
        if (!Array.isArray(allData) || allData.length === 0) throw new Error('Sin datos');
        ...
        err.innerHTML = '<b>Error:</b> ' + e.message;
      }
    }
    function init() { fillFilters(); applyFilters(); }
    function fillFilters() {
      const periodos = [...new Set(allData.map(r => r.periodo).filter(Boolean))].sort().reverse();
      const parcelas = [...new Set(allData.map(r => r.parcela).filter(Boolean))].sort();
    ...
    function filteredData() {
      ...
      return allData.filter(r => (!p || r.periodo == p) && (!pa || r.parcela == pa));
    }
    function applyFilters() { const data = filteredData(); renderStats(data); renderTable(data); renderCharts(data); }
    function renderStats(data) {
      const total = data.reduce((s, r) => s + parseFloat(r.monto || 0), 0);
      document.getElementById('stats').innerHTML = '<div class="stat-card"><div class="label">Total recaudado</div><div class="value blue">$' + total.toFixed(0) + '</div></div><div class="stat-card"><div class="label">Registros</div><div class="value">' + data.length + '</div></div><div class="stat-card"><div class="label">Periodos</div><div class="value">' + [...new Set(data.map(r => r.periodo))].length + '</div></div><div class="stat-card"><div class="label">Parcelas</div><div class="value">' + [...new Set(data.map(r => r.parcela))].length + '</div></div>';
    }
    function renderTable(data) {
      const tbody = document.getElementById('tableBody');
      if (data.length === 0) { tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;color:#9ca3af;padding:1.5rem">Sin registros</td></tr>'; return; }
      tbody.innerHTML = data.map(r => '<tr><td>' + (r.parcela || '') + '</td><td>' + (r.nombre || '') + '</td><td>' + (r.periodo || '') + '</td><td>' + (r.concepto || '') + '</td><td>$' + parseFloat(r.monto || 0).toFixed(0) + '</td><td>' + (r.fecha || '') + '</td></tr>').join('');
    }
    let chartPeriodos, chartParcelas;
    function renderCharts(data) { renderPeriodChart(data); renderParcelaChart(data); }
    function renderPeriodChart(data) {
      const groups = {};
      data.forEach(r => { const p = r.periodo || 'Sin periodo'; groups[p] = (groups[p] || 0) + parseFloat(r.monto || 0); });
      const labels = Object.keys(groups);
      const values = Object.values(groups);
      const ctx = document.getElementById('chartPeriodos').getContext('2d');
      if (chartPeriodos) chartPeriodos.destroy();
      chartPeriodos = new Chart(ctx, { type: 'bar', data: { labels, datasets: [{ label: 'Monto', data: values, backgroundColor: '#3b82f6', borderRadius: 4 }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true, ticks: { callback: v => '$' + v } } } } });
    }
    function renderParcelaChart(data) {
      const groups = {};
      data.forEach(r => { const p = r.parcela || 'Sin parcela'; groups[p] = (groups[p] || 0) + parseFloat(r.monto || 0); });
      const labels = Object.keys(groups);
      const values = Object.values(groups);
      const colors = ['#3b82f6','#10b981','#f59e0b','#ef4444','#8b5cf6','#ec4899','#14b8a6','#f97316','#6366f1','#84cc16'];
      const ctx = document.getElementById('chartParcelas').getContext('2d');
      if (chartParcelas) chartParcelas.destroy();
      chartParcelas = new Chart(ctx, { type: 'doughnut', data: { labels, datasets: [{ data: values, backgroundColor: colors.slice(0, labels.length) }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'bottom', labels: { boxWidth: 12, padding: 12, font: { size: 11 } } } } } });
    }
    loadData();
  </script>
</body>
